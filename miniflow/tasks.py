...
class Task(object):
    ...
    def prepare(self):
        import os
        task_dir = os.path.join(self.project.base_dir, "{0}".format(self.name))
    ...
    def logit(self, output):
        import os
        try:
            if self.project and os.path.exists(os.path.join(self.project.base_dir, "logs")):
                mode = 'w+'
                if os.path.exists(os.path.join(self.project.base_dir,"logs/{0}_logs.txt".format(self.name))):
                    mode = 'a+'
                with open(os.path.join(self.project.base_dir, "logs/{0}_logs.txt".format(self.name)),
                          mode=mode) as writer:
                    writer.write(output)
