...
def send_email(args,mail_message):
    ...
    mail_smtp_server = args.get('mail_smtp_server',None)
    mail_password = args.get('mail_password',None)
    mail_port = int(args.get('mail_port',587))
    mail_receiver = args.get('mail_receiver',None)
    ...
    with smtplib.SMTP(mail_smtp_server, mail_port) as server:
        server.ehlo()
        server.starttls()
